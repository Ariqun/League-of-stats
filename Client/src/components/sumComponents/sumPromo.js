...
export default class SummonerPromo extends Component {
	...
	updateCanvas = () => {
		...
		const percent = (wins * 100 / (wins + losses)).toFixed(2) / 100;
		...
		ctxWins.beginPath();
		ctxWins.arc(-100, 100, 80, 0, radians, false);
		ctxWins.lineWidth = 4;
		// ctxWins.closePath();
		ctxWins.stroke();
		...
		return(
			<div className="sum_rating">
				<div className="rank_block">
					...
					<span className="rank">{ruObj[tier.toLowerCase()]} {rank}</span>
				</div>
			</div>
		)
	}
	...
	createWinRateBlock = ({info}) => {
		...
		const percent = (wins * 100 / (wins + losses)).toFixed(2);
		return(
