...
import { addZeroFour } from '../../../../utils/actionsWithNums/addZeros';

const SkillDescr = ({
  spells, passive, keyID, currentSkill,
}) => {
  const [t] = useTranslation();
  const key = addZeroFour(keyID);
        ...
        <div className="video">
          <div className="wrapper_for_horizontal_borders">
            <div className="wrapper_for_vertical_borders">
              <video src={url} preload="auto" autoPlay loop muted="muted" />
            </div>
          </div>
        </div>
      </React.Fragment>
