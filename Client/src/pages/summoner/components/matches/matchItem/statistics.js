...
const Statistics = ({
  player, info, matchId, matchTypes,
}) => {
  const [t] = useTranslation();

  const {
    platformId, queueId, teams, gameStartTimestamp, gameDuration,
  } = info;
  const {
    kills, deaths, assists, totalMinionsKilled, neutralMinionsKilled, teamId,
  } = player;
...
const mapStateToProps = (state) => ({ matchTypes: state.matchTypes });
