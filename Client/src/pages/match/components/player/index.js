...
const Player = ({
  ...
  const result = players.map((player, i) => {
    const {
      summonerId, summonerName, championName, summoner1Id, summoner2Id, perks, individualPosition,
    } = player;
    const {
      kills, deaths, assists, totalMinionsKilled, neutralMinionsKilled, visionScore, goldEarned,
    } = player;
    ...
    const teamKills = players.reduce((acc, el) => acc += el.kills, 0);
